...
```{r}
tosearch<-c("Castilleja","Delphinium","Penstemon","Aquilegia","Mimulus","Gilia")
out <- name_backbone(name=tosearch,rank="genus")
head(out)
```

##Get all records with this genus key
```{r,eval=F}
dataout <- occ_search(scientificName = "Penstemon*|Castilleja*", coordinatestatus = TRUE, removeZeros = TRUE, limit = 110000,continent="north_america")
...
ggplot(data=dat,aes(x=year))  + geom_histogram() + theme_bw() + labs(x="Year",y="Records") + geom_vline(1955)
```
